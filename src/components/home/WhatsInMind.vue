<template>
  ...
  <Teleport to="body">
    <transition name="whatsonmind">
      <form v-if="showWhatsOnMindForm" class="fixed inset-0 z-[99] w-screen h-dvh bg-white grid overflow-hidden"
        ...
        <div class="grid gap-2 grid-cols-3 items-center justify-between px-2 pt-4 pb-1 border-b border-slate-300">
          <Icon icon="heroicons:x-mark-20-solid" width="28" color="#374151" @click="toggleWhatsOnMindForm"
          ...
          <Icon v-if="!hideHeader" icon="heroicons:ellipsis-horizontal" width="24" color="#374151"
        ...
        <header class="flex items-center gap-2 px-2 overflow-hidden transition-all duration-100"
          ...
          <UserAvatar :user customClass="w-10 h-10 rounded-full object-cover" goProfile />
          ...
          <div class="min-h-full flex flex-col">
            ...
            <div class="flex-1 min-h-[200px] px-2">
              <textarea type="text" placeholder="What's on your mind?" @focus="onFocusTextarea" v-model="postContent"
                class="w-full h-full outline-0 resize-none"></textarea>
            </div>
...
const handleFileChange = async (event: Event) => {
  ...
  files.forEach(file => {
    ...
    if (!allowedTypes.includes(file.type)) {
      ...
      return; // Salta este archivo y continúa con los demás
    }
